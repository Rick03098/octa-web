...
# 前端产品文档（2025-11-09）
OCTA是一款AI+风水产品，尽管在11-09的内测版本中它还是纯中文版本，但是实际上需要进行多语言的提前设置（字符串的放置位置），页面设计和逻辑上，它的主要页面按顺序包括（与Figma引入一一对应，每一页我给你的Figma都能找到因由，在backend-v1（后端仓库）中也能找到对应的API，然后里面会有一些并列的，你应该可以理解）：
...
3. 生日信息（用户输入生日信息）：这里的生日需要用滚轮（类似苹果的DataPicker），生日信息页的年月日选择，必须使用滚轮自身的显示效果，年月日最多出现选定的一个和上下若隐若现的两个；禁止在滚轮上方再放额外的芯片/文本同步显示，并确保滚轮位置与 Figma 素材像素级对齐
4. 出生时间信息（用户输入出生时间），同样禁止在滚轮上方再放额外的芯片/文本同步显示，时间最多出现选定的一个和上下若隐若现的两个
5. 出生地信息
6. 生理性别信息
