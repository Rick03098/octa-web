...
// [OUTPUT] DateWheelPicker组件, 适配52px高度容器的iOS风格滚轮选择器, 支持触摸和鼠标操作, 支持循环滚动
...
export function DateWheelPicker({
  ...
  // 但在循环跳转过程中不执行，避免干扰
  useEffect(() => {
    console.log('[DateWheelPicker] useEffect triggered:', {
      selectedValue,
    ...
    if (isJumpingRef.current) {
      console.log('[DateWheelPicker] useEffect skipped due to isJumping');
      return;
    }
    ...
    if (realIndex !== -1 && containerRef.current) {
      ...
      console.log('[DateWheelPicker] useEffect setting scrollTop:', {
        realIndex,
        loopIndex,
        scrollPosition
      });
  ...
  const handleEnd = () => {
    ...
    if (containerRef.current) {
      ...
      console.log('[DateWheelPicker] handleEnd:', {
        currentScrollTop,
        loopIndex,
        valuesLength: values.length,
        loopValuesLength: loopValues.length
      });
      ...
      console.log('[DateWheelPicker] looping check:', {
        loopIndex,
        isLooping,
        'loopIndex <= 0': loopIndex <= 0,
        'loopIndex > values.length': loopIndex > values.length
      });
      ...
      if (loopIndex <= 0) {
        ...
        console.log('[DateWheelPicker] Loop to last:', { realIndex, targetScroll, value: values[realIndex] });
      } else if (loopIndex > values.length) {
        // 虚拟尾部 -> 跳转到真实的第一个
        realIndex = 0;
        targetScroll = itemHeight;
        console.log('[DateWheelPicker] Loop to first:', { realIndex, targetScroll, value: values[realIndex] });
      }
      
      // 如果是循环跳转，先标记防止handleScroll和useEffect干扰
      if (isLooping) {
        console.log('[DateWheelPicker] Executing loop jump');
        isJumpingRef.current = true;
        ...
        setTimeout(() => {
          console.log('[DateWheelPicker] Reset isJumping, scrollTop:', containerRef.current?.scrollTop);
